__kernel void HeatTransfer(__global const float *matrixPresent, __global  float *matrixPrevious,  const int M,  const int N,  const float scaler)
{
	...
	if (x > 0 && x < (M - 1) && y > 0 && y < (N - 1)) {
		float x1 = matrixPresent[(x - 1) *N + y];// matrix[x-1][y]
